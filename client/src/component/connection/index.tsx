...
export const ConnectionComponent = (props: ConnectionProps) => {
  ...
  return (
    <div
      class={`absolute z-100 inset-0 w-screen h-screen text-gray-300 flex justify-center items-center bg-neutral-900 p-4 ${
        props.isConnected() ? "hidden" : "visible"
      }`}
    >
      {/* Main container */}
      <div class="connection-container duration-200 bg-neutral-800/65 rounded-3xl p-6 w-full lg:w-2xl space-y-4">
        ...
        <section class="px-1 space-y-4">
          ...
          <div class="flex flex-row gap-4">
            ...
            <button
              onClick={props.abort}
              class={`p-2 px-6 bg-neutral-700 w-auto text-gray-300 rounded-3xl transition-opacity duration-1000 ${
                props.isConnecting()
                  ? "opacity-100"
